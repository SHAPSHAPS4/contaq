...
router.post('/process', kbInjectionMiddleware, async (req, res) => {
  ...
  try {
      ...
      if (anthropicResp.ok) {
        try {
          ...
          if (jsonMatch) {
            ...
            if (rules.length) persistLearnedRules(rules);
            for (const p of (result.pattern_errors || [])) {
              persistPatternError(p.error_type, p.occurrences || 3, p.heightened_action || `Check ${p.error_type}`);
            }
          }
        } catch {}
      }
    ...
    if (learned_rules.length) persistLearnedRules(learned_rules);
    for (const p of pattern_errors) {
      persistPatternError(p.error_type, p.occurrences, p.heightened_action);
    }
