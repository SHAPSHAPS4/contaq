...
/* Excel export — builds a professional .xlsx with header, meta, items table, and totals */
function downloadQuoteExcel(tenderId) {
  var t = TENDERS.find(function(x) { return x.id === tenderId; });
  if (!t) { showToast('Quote not found.', 'error'); return; }
  if (typeof XLSX === 'undefined') {
    showToast('Excel library still loading — try again in a moment.', 'error'); return;
  }
  var cl = CLIENTS.find(function(c) { return c.id === t.client; });
  var clientName = cl ? cl.name : (t.clientName || 'Client');
  var clientAddr = cl ? (cl.address || '') : '';
  ...
  var rows = [];
  var merges = [];
  var r = 0; /* current row index */

  /* Row 0: Company header */
  rows.push(['CONTRAQ — M&E Estimation & Contract Management', '', '', '', '', '', '']);
  merges.push({ s: { r: r, c: 0 }, e: { r: r, c: 6 } });
  r++;

  /* Row 1: Document title */
  rows.push(['Quotation: ' + t.ref, '', '', '', '', '', '']);
  merges.push({ s: { r: r, c: 0 }, e: { r: r, c: 6 } });
  r++;
  ...
  /* Rows 3-8: Meta info */
  rows.push(['Client:', clientName, '', 'Quote Ref:', t.ref, '', '']);
  rows.push(['Address:', clientAddr || '\u2014', '', 'Project:', t.name, '', '']);
  rows.push(['Date:', dateStr, '', 'Status:', (t.status || 'open').charAt(0).toUpperCase() + (t.status || 'open').slice(1), '', '']);
  rows.push(['Enquiry:', t.enquiry || '\u2014', '', '', '', '', '']);
  r += 4;
  ...
  /* Summary row */
  if (items.length > 0) {
    rows.push(['Line Items:', items.length, '', 'Avg Confidence:', (meta.avgConfidence || '\u2014') + '%', 'Total Value:', grandTotal]);
    r++;
    rows.push([]); r++;
  }
  ...
  /* Table header */
  var hdrRow = r;
  rows.push(['Ref', 'Description', 'Group', 'Qty', 'Unit', 'Rate (\u00A3)', 'Total (\u00A3)']);
  r++;
  ...
  if (items.length > 0) {
    items.forEach(function(li) {
      rows.push([
        li.ref || '',
        li.desc || '',
        li.unit_equip || li.service || 'General',
        li.qty || 0,
        li.unit || 'nr',
        li.rate || 0,
        li.total || 0
  ...
  /* Terms */
  rows.push([]); r++;
  rows.push(['Terms & Conditions: Valid for 30 days. Prices exclusive of VAT. Payment terms: ' + (cl && cl.creditTerms ? cl.creditTerms + ' days' : '30 days') + '. Works per relevant British Standards and project specifications.']);
  merges.push({ s: { r: r, c: 0 }, e: { r: r, c: 6 } });
  r++;

  if (items.length > 0 && t.aiMetadata) {
    rows.push(['AI-generated estimate (KB v' + (t.aiMetadata.kbVersion || '') + '). Verify before commercial use.']);
    merges.push({ s: { r: r, c: 0 }, e: { r: r, c: 6 } });
    r++;
  }

  rows.push([]); r++;
  rows.push(['Generated by Contraq \u00B7 contraq.co.uk']);
  merges.push({ s: { r: r, c: 0 }, e: { r: r, c: 6 } });
  ...
  for (var R = hdrRow + 1; R <= range.e.r; R++) {
    var rateCell = XLSX.utils.encode_cell({ r: R, c: 5 });
    var totalCell = XLSX.utils.encode_cell({ r: R, c: 6 });
    var qtyCell = XLSX.utils.encode_cell({ r: R, c: 3 });
    if (ws[rateCell] && typeof ws[rateCell].v === 'number') ws[rateCell].z = '\u00A3#,##0.00';
    if (ws[totalCell] && typeof ws[totalCell].v === 'number') ws[totalCell].z = '\u00A3#,##0.00';
    if (ws[qtyCell] && typeof ws[qtyCell].v === 'number') ws[qtyCell].z = '#,##0';
  }

  /* Build workbook and trigger download */
  var wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Quote ' + t.ref);
  XLSX.writeFile(wb, safeRef + '.xlsx');
